...
void CommandLine::RunSetup() {
  ...
  MySerial.println(F("\n\n--------------------------------\n"));
  MySerial.println(F("         ESP32 Marauder      \n"));
  MySerial.println("            " + version_number + "\n");
  MySerial.println(F("       By: justcallmekoko\n"));
  MySerial.println(F("--------------------------------\n\n"));
...
void CommandLine::runCommand(String input) {
  ...
  if (cmd_args.get(0) == HELP_CMD) {
    ...
    MySerial.println(HELP_LED_CMD);
    MySerial.println(HELP_GPS_DATA_CMD);
    MySerial.println(HELP_GPS_CMD);
    MySerial.println(HELP_NMEA_CMD);
    
    // WiFi sniff/scan
    MySerial.println(HELP_EVIL_PORTAL_CMD);
    ...
    #ifdef HAS_GPS
      MySerial.println(HELP_WARDRIVE_CMD);
    #endif
    ...
    // Bluetooth sniff/scan
      ...
      MySerial.println(HELP_BT_SPAM_CMD);
      //MySerial.println(HELP_BT_SWIFTPAIR_SPAM_CMD);
      //MySerial.println(HELP_BT_SAMSUNG_SPAM_CMD);
      //MySerial.println(HELP_BT_SPAM_ALL_CMD);
      #ifdef HAS_GPS
        MySerial.println(HELP_BT_WARDRIVE_CMD);
      #endif
  ...
  if (cmd_args.get(0) == STOPSCAN_CMD) {
    ...
    if(old_scan_mode == WIFI_SCAN_GPS_NMEA)
      MySerial.println("END OF NMEA STREAM");
    else if(old_scan_mode == WIFI_SCAN_GPS_DATA)
      MySerial.println("Stopping GPS data updates");
    else
    ...
    // If we don't do this, the text and button coordinates will be off
      ...
      if (gps_obj.getGpsModuleStatus()) {
        MySerial.println("Getting GPS Data. Stop with " + (String)STOPSCAN_CMD);
      ...
      if (gps_obj.getGpsModuleStatus()) {
        ...
        if (get_arg != -1) {
          ...
          if (gps_info == "fix")
            MySerial.println("Fix: " + gps_obj.getFixStatusAsString());
          else if (gps_info == "sat")
            MySerial.println("Sats: " + gps_obj.getNumSatsString());
          else if (gps_info == "lat")
            MySerial.println("Lat: " + gps_obj.getLat());
          else if (gps_info == "lon")
            MySerial.println("Lon: " + gps_obj.getLon());
          else if (gps_info == "alt")
            MySerial.println("Alt: " + (String)gps_obj.getAlt());
          else if (gps_info == "accuracy")
            MySerial.println("Accuracy: " + (String)gps_obj.getAccuracy());
          else if (gps_info == "date")
            MySerial.println("Date/Time: " + gps_obj.getDatetime());
          else if (gps_info == "text"){
            MySerial.println(gps_obj.getText());
          }
            ...
            if(notparsed_arg == -1 && notimp_arg == -1 && recd_arg == -1){
              gps_obj.sendSentence(MySerial, gps_obj.generateGXgga().c_str());
              gps_obj.sendSentence(MySerial, gps_obj.generateGXrmc().c_str());
            }
            else if(notparsed_arg == -1 && notimp_arg == -1)
              MySerial.println(gps_obj.getNmea());
            else if(notparsed_arg == -1)
              MySerial.println(gps_obj.getNmeaNotimp());
            else
              MySerial.println(gps_obj.getNmeaNotparsed());
          }
          else
            MySerial.println("You did not provide a valid argument");
        }
        else if(nmea_arg != -1){
          ...
          if (nmea_type == "native" || nmea_type == "all" || nmea_type == "gps" || nmea_type == "glonass"
            ...
            MySerial.println("GPS Output Type Set To: " + nmea_type);
          }
          else
            MySerial.println("You did not provide a valid argument");
        }
        else if(cmd_args.size()>1)
          MySerial.println("You did not provide a valid flag");
        else
          MySerial.println("You did not provide an argument");
      }
    #endif
  }
      ...
      if (gps_obj.getGpsModuleStatus()) {
          ...
          menu_function_obj.changeMenu(&menu_function_obj.gpsInfoMenu);
        #endif
        MySerial.println("NMEA STREAM FOLLOWS");
  ...
  // LED command
    ...
    int pat_arg = this->argSearch(&cmd_args, "-p");
      ...
      if (hex_arg != -1) {
        ...
        //MySerial.println(g);
        //MySerial.println(b);
        flipper_led.setColor(r, g, b);
        flipper_led.setMode(MODE_CUSTOM);
      }
      else if (pat_arg != -1) {
        String pat_name = cmd_args.get(pat_arg + 1);
        pat_name.toLowerCase();
        if (pat_name == "rainbow") {
          flipper_led.setMode(MODE_RAINBOW);
        }
      }
    #else
      MySerial.println("This hardware does not support neopixel");
    #endif
  }
  // ls command
    ...
    #else
      MySerial.println("SD support disabled, cannot use command");
      return;
    #endif
  }
  ...
  if (!wifi_scan_obj.scanning()) {
    ...
    // Wardrive
        ...
        if (gps_obj.getGpsModuleStatus()) {
          ...
          if (sta_sw == -1) {
            MySerial.println("Starting Wardrive. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          else {MySerial.println("Starting Station Wardrive. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          MySerial.println("GPS Module not detected");
      #else
        MySerial.println("GPS not supported");
      #endif
    }
    // AP Scan
      ...
      if (cmd_sw != -1) {
        ...
        if (et_command == "start") {
          MySerial.println("Starting Evil Portal. Stop with " + (String)STOPSCAN_CMD);
          #ifdef HAS_SCREEN
            display_obj.clearScreen();
            menu_function_obj.drawStatusBar();
          #endif
          if (html_sw != -1) {
            ...
            MySerial.println("Set html file as " + evil_portal_obj.target_html_name);
          }
          //else {
    ...
    if (cmd_args.get(0) == BT_SNIFF_CMD) {
      ...
      if (bt_type_sw != -1) {
        ...
        if (bt_type == "apple") {
          #ifdef HAS_BT
            MySerial.println("Starting Sour Apple attack. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          #ifdef HAS_BT
            MySerial.println("Starting Swiftpair Spam attack. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          #ifdef HAS_BT
            MySerial.println("Starting Samsung Spam attack. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          #ifdef HAS_BT
            MySerial.println("Starting Google Spam attack. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          #ifdef HAS_BT
            MySerial.println("Starting BT Spam All attack. Stop with " + (String)STOPSCAN_CMD);
            #ifdef HAS_SCREEN
              display_obj.clearScreen();
              menu_function_obj.drawStatusBar();
            #endif
          ...
          MySerial.println("You did not specify a correct spam type");
        }
      }
    }
    ...
    // Wardrive
          ...
          if (gps_obj.getGpsModuleStatus()) {
            ...
            if (cont_sw == -1) {
              MySerial.println("Starting BT Wardrive. Stop with " + (String)STOPSCAN_CMD);
              #ifdef HAS_SCREEN
                display_obj.clearScreen();
                menu_function_obj.drawStatusBar();
              #endif
            ...
            else {MySerial.println("Starting Continuous BT Wardrive. Stop with " + (String)STOPSCAN_CMD);
              #ifdef HAS_SCREEN
                display_obj.clearScreen();
                menu_function_obj.drawStatusBar();
              #endif
    ...
    if (cmd_args.get(0) == UPDATE_CMD) {
      ...
      if (sd_sw != -1) {
          ...
          sd_obj.runUpdate();
        #else
          MySerial.println("SD card support disabled. Cannot perform SD Update");
          return;
        #endif
      }
    }
