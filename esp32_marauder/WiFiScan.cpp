...
extern "C" {
  ...
  NimBLEAdvertisementData WiFiScan::GetUniversalAdvertisementData(EBLEPayloadType Type) {
    ...
    switch (Type) {
      ...
      default: {
        MySerial.println("Please Provide a Company Type");
        break;
      }
    }
...
void WiFiScan::initWiFi(uint8_t scan_mode) {
  ...
  if (scan_mode != WIFI_SCAN_OFF) {
    //MySerial.println(F("Initializing WiFi settings..."));
    this->changeChannel();
    ...
    //MySerial.println(F("Initialization complete"));
  }
}
...
void WiFiScan::startWiFiAttacks(uint8_t scan_mode, uint16_t color, String title_string) {
  ...
  this->wifi_initialized = true;
    ...
    // uncomment below
    flipper_led.setMode(MODE_ATTACK);
...
void WiFiScan::StopScan(uint8_t scan_mode)
{
  ...
  (currentScanMode == BT_SCAN_SKIMMERS))
  {
    ...
    #endif
    //MySerial.print("display_buffer->size(): ");
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color)
{
  startLog("evil_portal");
    ...
    // uncomment below
    flipper_led.setMode(MODE_SNIFF);
  #elif defined(XIAO_ESP32_S3)
    xiao_led.sniffLED();
  #elif defined(MARAUDER_M5STICKC)
    stickc_led.sniffLED();
  ...
  //  MySerial.println("Could not successfully start EvilPortal. Setting WIFI_SCAN_OFF...");
  //  this->StartScan(WIFI_SCAN_OFF, TFT_MAGENTA);
  //  return;
  //}
  //else
  //  MySerial.println("Setup EvilPortal. Current mode: " + this->currentScanMode);
  this->wifi_initialized = true;
  initTime = millis();
}
...
void WiFiScan::RunGPSInfo() {
    ...
    MySerial.println("Refreshing GPS Data on screen...");
    #ifdef HAS_SCREEN
    ...
    // Display to serial
    MySerial.println("==== GPS Data ====");
    if (gps_obj.getFixStatus())
      MySerial.println("  Good Fix: Yes");
    else
      MySerial.println("  Good Fix: No");
      
    if(text != "") MySerial.println("      Text: " + text);

    MySerial.println("Satellites: " + gps_obj.getNumSatsString());
    MySerial.println("  Accuracy: " + (String)gps_obj.getAccuracy());
    MySerial.println("  Latitude: " + gps_obj.getLat());
    MySerial.println(" Longitude: " + gps_obj.getLon());
    MySerial.println("  Altitude: " + (String)gps_obj.getAlt());
    MySerial.println("  Datetime: " + gps_obj.getDatetime());
  #endif
}
...
void WiFiScan::RunGPSNmea() {
    ...
    if(buffer && queue_enabled){
      ...
      if(size){
        ...
        for(int i=0;i<size;i++){
          ...
          MySerial.println(line.sentence);
      ...
      if(nmea_sentence != "" && nmea_sentence != old_nmea_sentence){
        ...
        MySerial.println(nmea_sentence);
      }
    ...
    gps_obj.sendSentence(MySerial, gxgga.c_str());
    gps_obj.sendSentence(MySerial, gxrmc.c_str());
    ...
    if (gps_obj.getGpsModuleStatus()) {
      ...
      if (n > 0) {
        for (int i = 0; i < n; i++) {
          ...
          MySerial.print((String)this->mac_history_cursor + " | " + wardrive_line);
...
void WiFiScan::apSnifferCallbackFull(void* buf, wifi_promiscuous_pkt_type_t type) {  
  ...
  if (type == WIFI_PKT_MGMT)
  {
    ...
    if ((snifferPacket->payload[0] == 0x80) && (buf == 0))
    {
      ...
      for (int i = 0; i < access_points->size(); i++) {
        mac_match = true;
        //MySerial.print("Checking ");
        //MySerial.print(addr);
        //MySerial.println(" against " + (String)access_points->get(i).essid);

        
        for (int x = 0; x < 6; x++) {
          //MySerial.println((String)snifferPacket->payload[x + 10] + " | " + (String)access_points->get(i).bssid[x]);
          if (snifferPacket->payload[x + 10] != access_points->get(i).bssid[x]) {
            mac_match = false;
            //MySerial.println("MACs do not match");
            break;
          }
        }
        if (mac_match) {
      ...
      if (!in_list) {
        ...
        MySerial.print(" ");
        MySerial.print(esp_get_free_heap_size());
      ...
      if (wifi_scan_obj.currentScanMode == WIFI_SCAN_SIG_STREN) {
        ...
        for (int i = 0; i < access_points->size(); i++) {
          if (access_points->get(i).selected) {
            ...
            if (found) {
              //MySerial.println("Received beacon from " + access_points->get(i).essid + ". Checking RSSI...");
        ...
        buffer_obj.append(snifferPacket, len);
          ...
          if (gps_obj.getGpsModuleStatus()) {
            ...
            if (do_save) {
              if (n > 0) {
                for (int i = 0; i < n; i++) {
                  MySerial.printf("%-32.32s", WiFi.SSID(i).c_str());
                  MySerial.print(" -> ");
                  MySerial.println(wifi_scan_obj.security_int_to_string(WiFi.encryptionType(i)).c_str());
                }
              }
              ...
              MySerial.println(wardrive_line);
              //buffer_obj.append(wardrive_line);
            }
          }
        #endif
    ...
    int temp_len = display_string.length();
      ...
      //MySerial.print(" ");
        ...
        if (display_obj.display_buffer->size() == 0)
        {
          ...
          display_obj.display_buffer->add(display_string);
          display_obj.loading = false;
          MySerial.println(display_string);
        }
      #endif
    #endif
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= 320; x_pos = x_pos)
    {
      ...
      if (currentTime - initTime >= GRAPH_REFRESH) {
        //MySerial.println("-----------------------------------------");
        //MySerial.println("Time elapsed: " + (String)(currentTime - initTime) + "ms");
        x_pos += x_scale;
        initTime = millis();
        ...
        //MySerial.println("num_beacon: " + (String)num_beacon);
        //MySerial.println("num_deauth: " + (String)num_deauth);
        //MySerial.println(" num_probe: " + (String)num_probe);
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if (currentScanMode == WIFI_SCAN_ACTIVE_EAPOL)
  {
    ...
    {
      initTime = millis();
      //My.print("packets/sec: ");
      //MySerial.println(packets_sent);
      String displayString = "";
      String displayString2 = "";
      displayString.concat(text18);
      displayString.concat(packets_sent);
